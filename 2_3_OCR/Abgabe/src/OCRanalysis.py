...
class OCRanalysis:
    ...
    def run(self, img_path, tgtCharRow: int, tgtCharCol: int) -> int:
        ...
        # Return hitCount for better testing
        return hitCount
...
def is_empty_column(in_img, height, col_idx, BG_val):
    """
    Checks if a column in the image contains only background pixels.

    :param in_img: The input image array
    :param height: Height of the image
    :param col_idx: Index of the column to check
    :param BG_val: Background pixel value
    :return: True if the entire column contains only background pixels, False otherwise
    """
...
def is_empty_row(in_img, width, row_idx, BG_val):
    """
    Checks if a row in the image contains only background pixels.

    :param in_img: The input image array
    :param width: Width of the image
    :param row_idx: Index of the row to check
    :param BG_val: Background pixel value
    :return: True if the entire row contains only background pixels, False otherwise
    """
...
def split_characters_vertically(row_image, BG_val, orig_img, row_start_y):
    """
    This function scans a row of characters and identifies sub-regions corresponding to each character.

    :param row_image: A cropped portion of the image containing a single row of characters
    :param BG_val: The background color value
    :param orig_img: The original full image
    :param row_start_y: The Y-coordinate in the original image where the character row starts
    :return: List of 'SubImageRegion' objects
    """
...
def minimize_character_bounding_height(inImg: np.array, region: SubImageRegion, FG_val: int) -> SubImageRegion:
    """
    This function applies the fire-through method to a given subregion horizontally. Such that the resulting SubImageRegion
    has minimal height.

    :param inImg: The input image (complete image)
    :param region: The SubImageRegion, that represents a single character, that should be minimized.
    :param FG_val: The foreground color value
    :return: A new SubImageRegion, that contains the minimized character region
    """
...
def split_characters(in_img, width, height, BG_val, FG_val):
    """
    Splits an image into a nested list of character regions, grouped by rows.

    :param in_img: The input image
    :param width: The width of the image
    :param height: The height of the image
    :param BG_val: The background color value
    :param FG_val: The foreground color value used to distinguish characters from the background
    :return: A list of lists, where each inner list contains 'SubImageRegion' objects
        representing characters found in a single row.
    """
    char_regions_list = []
    current_row_img = []
    row_start_y = None
        ...
        if is_empty_row(in_img, width, y, BG_val):
            if current_row_img:
                ...
                char_row_regions_list = split_characters_vertically(
                    row_img, BG_val, in_img, row_start_y
                )
...
def highlight_letters(orig_img, sub_image_regions):
    """
    Highlights the character regions in a given image.

    :param orig_img: The original image in greyscale
    :param sub_image_regions: A list of 'SubImageRegion' objects
    :return: A color image with highlighted character regions
    """
...
def is_matching_char(curr_feature_arr, ref_feature_arr, norm_feature_arr):
    ...
    :return: True if the character is matching the reference character, False otherwise.
    """
...
"""
    This feature describes the absolut width (result needs to be normalized)
"""
...
"""
    This feature describes the absolut height (result needs to be normalized)
"""
...
"""
    This Feature measures describes the ration of width to height relatively
"""
...
"""
    This feature measures the ratio between foreground and background pixels based on their
    area. (e.g. "I" would result in a hight value, because the are of foreground in the bounding-box
    is high compared to background. But "O" would result in a lower value, because the measuring is relativ.
"""
...
"""
    This feature measures the vertically asymmetry. That means, that foreground pixels on
    the left hand side are less worth, than foreground pixels on right hand side. Such that
    there is quiet large difference between for example b and d. 
"""
...
"""
    This feature measures the horizontal asymmetry. That means, that foreground pixels at
    the bottom are less worth, than foreground pixels at top. 
"""
...
"""
    This function is used to test the implemented ocr against a given test image.
"""
def main():
    ...
    # Define expected results for given input image each tupel containes (character, ref_row, ref_col, expected_occourence)
    testCases = [
        ('e', 5, 0, 169),
        ('n', 0, 3, 115),
        ('s', 1, 3, 102),
