...
def split_characters_vertically(row_image, BG_val, FG_val, orig_img):
    ...
    for i, img_col in enumerate(row_image):
        if is_empty_column(row_image, len(row_image), i, BG_val):
            # found a full column
            if current_col_img:
                return_char_arr.append(current_col_img)
                current_col_img = []
        else:
            current_col_img.append(img_col)
...
def split_characters(in_img, width, height, BG_val, FG_val):
    ...
    for i, img_row in enumerate(in_img):
        if is_empty_row(in_img, width, i, BG_val):
            # found a full row -> split characters vertically and add it to the result
            if current_row_img:
                row = current_row_img
                single_character_list = split_characters_vertically(row, BG_val, FG_val, in_img)
                return_char_matrix.append(np.ndarray(single_character_list).copy())
                current_row_img = []
        else:
            current_row_img.append(img_row)

    cv2.imshow("split_characters", return_char_matrix)
    cv2.waitKey(0)
