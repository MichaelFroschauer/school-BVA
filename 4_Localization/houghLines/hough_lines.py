        ...
        for x in range(0, inImgWidth):
            ...
            if edgeVal >= minThresh:
                ...
                for degreeIdx in range(0, accBuffWidth):
                    ...
                    sinVal = math.sin(rad)
                    cosVal = math.cos(rad)
                    # now calculate radius from angle via polar coordinate formular
                    radiusIdx = int(x * cosVal + y * sinVal + 0.5)
                    if radiusIdx >= 1.0:
                        accumulationBuffer[radiusIdx, degreeIdx] += 1



    print('accumulation buffer calculated with #FG = ' + str(countFG) +
          ' ratio = ' + str(countFG / (inImgWidth * inImgHeight) * 100.0))
  ...
  for degreeIdx in range(0, accBuffWidth):
      for radiusIdx in range(0, accBuffHeight):
          ...
          if currVal > maxVal:
              ...
              print('new MAX found with ' + str(maxVal) +
                    ' at radius = ' + str(radiusIdx) +
                    ' at degree = ' + str(degreeIdx))
...
if __name__ == "__main__":    
    ...
    #imgInPath = "./lines1.png"
    #imgInPath = "./lines2.png"
